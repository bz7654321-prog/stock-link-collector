...
TARGET_STOCKS_BY_CHANNEL = {
    ...
    "@디에스경제타임즈": [],
    "@DSnews77": [],
    "@Power_bus2": ["알테오젠", "196170", "클로봇", "466100", "삼성중공업", "010140", "채비", "에이비엘바이오"],
    "@문선생_경제교실": ["펩트론", "087010", "알테오젠", "196170", "에이비엘바이오"], 
    "@MIRACLEG372": ["대한전선", "고영", "대우건설",] 
}
...
def main():
    ...
    message = """아래 유튜브 영상 링크들을 확인하고, 바쁜 주식 투자자를 위해 핵심만 요약해줘.

[작성 원칙]
1. 종목코드는 적지 말고 종목명만 사용해.
2. 각 영상 링크 옆에 [🎯관심종목]이 지정된 경우, 무조건 해당 종목과 관련된 전략만 집중적으로 파고들어서 요약해. 지정되지 않은 영상은 핵심 추천 종목을 알아서 찾아 요약해.
3. 스쳐 지나가듯 단순 언급된 종목은 무조건 요약에서 빼.
4. 매수가, 목표가, 손절가, 세력목표가가 영상에 나오면 절대 누락하지 말고 유튜버가 말한 뉘앙스(ex. ~원 부근 매도, ~원 아래서 모아가라)를 그대로 살려서 적어.
5. "급등", "상한가" 같은 자극적인 말은 빼고 객관적으로 요약해.

[출력 양식 (영상 1개당 반복)]
📺 영상 제목 (채널명)
- 핵심 요약: (2~3줄)
- 추천 종목: [종목명]
- 가격 전략: 진입가 / 목표가 / 세력목표가 / 손절가 (언급된 것만)
- 상승 논리: (불릿 1~2개)
